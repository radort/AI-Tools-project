...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/admin/two-factor/confirm`, {
      method: 'POST',
      headers: {
    ...
    console.error('Admin 2FA confirm proxy error:', error)

    if (error.name === 'AbortError') {
      return NextResponse.json(
        { message: 'Admin 2FA confirmation request timed out. Please try again.' },
        { status: 504 }
      )
    }
